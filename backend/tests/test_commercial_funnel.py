...
def _public_inquiry(client, organization_id: str, slug: str, *, name: str, email: str, phone: str) -> None:
    assert_response(
        client.post(
            f"/api/public/sites/{organization_id}/properties/{slug}/inquiries",
            json={
                "name": name,
                "email": email,
                "phone": phone,
                "preferred_contact": "whatsapp",
                "message": "Quero conhecer o imóvel e avaliar uma proposta de locação.",
                "consent": True,
                "website": "",
            },
        ),
...
def test_site_lead_flows_through_visit_proposal_contract_and_closes_competitors(client, identity):
    owner = create_person(
        client,
        name="Proprietário Funil",
        document="81818181818",
    ...
    principal = by_email["principal@example.com"]
    competitor = by_email["concorrente@example.com"]

    visit_start = datetime.combine(_next_weekday(), time(hour=17), tzinfo=timezone.utc)
    visit = assert_response(
        client.post(
            f"/api/crm/site-inquiries/{principal['id']}/visits",
            json={
                "starts_at": visit_start.isoformat(),
                "duration_minutes": 60,
                "notes": "Visita criada pelo funil comercial automatizado.",
            },
        ),
        201,
    ).json()
    ...
    assert funnel["inquiry"]["status"] == "visit_scheduled"
    assert funnel["person"] is not None
    assert funnel["person"]["name"] == "Interessado Principal"
    linked_person_id = funnel["person"]["id"]

    assert SessionLocal is not None
    with SessionLocal() as db:
        person = db.get(Person, UUID(linked_person_id))
        assert person is not None
        tenant_role = db.scalar(select(PersonRole).where(PersonRole.person_id == person.id, PersonRole.role_key == "tenant"))
        assert tenant_role is not None and tenant_role.is_active is True
        agenda = db.get(AgendaTask, UUID(visit["agenda_task_id"]))
        assert agenda is not None
        assert agenda.source_module == "crm"
        assert agenda.source_type == "commercial_visit"
        assert agenda.source_id == visit["id"]
        assert agenda.status == "pending"

    completed = assert_response(
        client.patch(f"/api/crm/visits/{visit['id']}", json={"status": "completed", "notes": None})
    ).json()
    assert completed["status"] == "completed"
    with SessionLocal() as db:
        agenda = db.get(AgendaTask, UUID(visit["agenda_task_id"]))
        assert agenda is not None and agenda.status == "completed"

    proposal_start = date.today() + timedelta(days=20)
    proposal_payload = {
        "rent_amount": "1950.00",
        "start_date": proposal_start.isoformat(),
        "term_months": 30,
        "guarantee_type": "insurance",
        "notes": "Condição negociada no atendimento comercial.",
    }
    proposal = assert_response(
        client.post(f"/api/crm/site-inquiries/{principal['id']}/proposals", json=proposal_payload),
        201,
    ).json()
    competing_proposal = assert_response(
        client.post(
            f"/api/crm/site-inquiries/{competitor['id']}/proposals",
            json={**proposal_payload, "rent_amount": "2000.00", "notes": "Proposta concorrente."},
        ),
        201,
    ).json()
    assert proposal["person_id"] == linked_person_id
    assert competing_proposal["status"] == "submitted"

    proposal = assert_response(
        client.patch(f"/api/crm/proposals/{proposal['id']}", json={"status": "accepted", "reason": None})
    ).json()
    competing_proposal = assert_response(
        client.patch(f"/api/crm/proposals/{competing_proposal['id']}", json={"status": "accepted", "reason": None})
    ).json()
    assert proposal["status"] == "accepted"
    assert competing_proposal["status"] == "accepted"

    conversion = assert_response(
        client.post(f"/api/crm/proposals/{proposal['id']}/convert-to-lease"),
        201,
    ).json()
    assert conversion["lease_status"] == "draft"
    assert conversion["lease_code"].startswith("LOC-")
    assert conversion["proposal"]["status"] == "converted"

    # Proposta/contrato em andamento não retira o imóvel do estoque.
    ...
    assert current_property["status"] == "available"
    assert current_property["publication_enabled"] is True

    signed = _run_signature_flow(client, kind="lease", contract_id=conversion["lease_contract_id"])
    ...
    assert principal_funnel["inquiry"]["status"] == "won"
    assert principal_funnel["proposals"][0]["status"] == "won"
    assert principal_funnel["proposals"][0]["lease_code"] == conversion["lease_code"]
    assert competitor_funnel["inquiry"]["status"] == "lost"
    assert competitor_funnel["proposals"][0]["status"] == "rejected"
    assert competitor_funnel["proposals"][0]["closed_reason"] == "Imóvel locado por outra proposta."
