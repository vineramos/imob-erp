...
def upgrade() -> None:
    ...
    op.add_column(
        ...
        sa.Column("signing_metadata", postgresql.JSONB(astext_type=sa.Text()), server_default=sa.text("'{}'::jsonb"), nullable=False),
    )
    op.add_column(
        "administration_contracts",
...
def downgrade() -> None:
    ...
    op.drop_column("administration_contracts", "signing_metadata")
