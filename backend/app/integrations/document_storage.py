...
class GcsDocumentStorage:
    ...
    def status(self, *, probe: bool = False) -> DocumentStorageStatus:
        if not self.configured:
            return DocumentStorageStatus(provider="gcs", configured=False, reachable=None, bucket=None, message="Bucket de documentos ainda não configurado.", checked_at=datetime.now(timezone.utc))
        if not probe:
            return DocumentStorageStatus(provider="gcs", configured=True, reachable=None, bucket=self.bucket_name, message="Bucket configurado. Execute o teste para validar acesso da service account.", checked_at=datetime.now(timezone.utc))
        try:
            client = storage.Client(); bucket = client.bucket(self.bucket_name); bucket.reload()
            return DocumentStorageStatus(provider="gcs", configured=True, reachable=True, bucket=self.bucket_name, message="Storage de documentos acessível pela service account do Cloud Run.", checked_at=datetime.now(timezone.utc))
        except NotFound:
        ...
        return DocumentStorageStatus(provider="gcs", configured=True, reachable=False, bucket=self.bucket_name, message=message, checked_at=datetime.now(timezone.utc))
    ...
    def upload_bytes(self, *, object_name: str, content: bytes, content_type: str) -> str:
        ...
        try:
            client = storage.Client(); bucket = client.bucket(self.bucket_name); blob = bucket.blob(object_name)
