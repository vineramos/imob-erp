...
def _annual(db: Session, organization_id: UUID, year: int, party_type: str, person_id: UUID) -> AnnualIncomeReport:
    try:
        person, raw_lines, allocation_method = annual_income_values(db, organization_id=organization_id, year=year, party_type=party_type, person_id=person_id)
...
def dre(start_date: date = Query(...), end_date: date = Query(...), regime: str = Query(default="cash", pattern="^(cash|competence)$"), context: UserContext = Depends(require_permission("reports.view")), db: Session = Depends(get_db)) -> DreReport:
    return _dre(db, context.user.organization_id, start_date, end_date, regime)


@router.get("/dre.pdf")
def dre_pdf(start_date: date = Query(...), end_date: date = Query(...), regime: str = Query(default="cash", pattern="^(cash|competence)$"), context: UserContext = Depends(require_permission("reports.view")), db: Session = Depends(get_db)) -> Response:
    ...
    return Response(content=payload, media_type="application/pdf", headers={"Content-Disposition": f'inline; filename="dre-{start_date:%Y-%m-%d}-{end_date:%Y-%m-%d}.pdf"'})


@router.get("/overview", response_model=FinanceReportOverview)
def overview(start_date: date = Query(...), end_date: date = Query(...), context: UserContext = Depends(require_permission("reports.view")), db: Session = Depends(get_db)) -> FinanceReportOverview:
    if end_date < start_date:
        raise HTTPException(status_code=422, detail="Período inválido.")
    ...
    return FinanceReportOverview(
        ...
        overdue_count=len(overdue), paid_charges=len(paid), open_charges=len(open_items),
    )


@router.get("/annual-income", response_model=AnnualIncomeReport)
def annual_income(year: int = Query(..., ge=2000, le=2200), party_type: str = Query(default="tenant", pattern="^(tenant|owner)$"), person_id: UUID = Query(...), context: UserContext = Depends(require_permission("reports.view")), db: Session = Depends(get_db)) -> AnnualIncomeReport:
    return _annual(db, context.user.organization_id, year, party_type, person_id)


@router.get("/annual-income.pdf")
def annual_income_pdf(year: int = Query(..., ge=2000, le=2200), party_type: str = Query(default="tenant", pattern="^(tenant|owner)$"), person_id: UUID = Query(...), context: UserContext = Depends(require_permission("reports.view")), db: Session = Depends(get_db)) -> Response:
