...
export function AgendaMeetingCenter({ open, onClose, context, startMode = 'inbox', onChanged }: Props) {
  ...
  function toggleParticipant(id: string) { setParticipants(current => current.includes(id) ? current.filter(item => item !== id) : [...current, id]) }
  ...
  async function sendRequest() {
    ...
    try {
      ...
      setTitle(''); setDescription(''); setParticipants([]); setDraftOptions([]); setMode('inbox'); await load(); onChanged()
  ...
  return <div className="portfolio-modal-backdrop" onMouseDown={event => { if (event.currentTarget === event.target && !saving) onClose() }}>
    <section className="panel portfolio-modal agenda-smart-modal agenda-meeting-modal" role="dialog" aria-modal="true">
      ...
      <div className="agenda-smart-body">
        ...
        {(mode === 'request' || mode === 'find') && <div className="agenda-meeting-compose"><div className="agenda-participant-picker"><div className="agenda-section-heading"><div><span className="eyebrow">Participantes</span><h3>{participants.length} selecionado(s)</h3></div><select value={departmentFilter} onChange={event => setDepartmentFilter(event.target.value)}><option value="all">Todos os setores</option>{context.departments.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select></div><div className="agenda-people-grid">{directory.map(user => <label className={participants.includes(user.id) ? 'selected' : ''} key={user.id}><input type="checkbox" checked={participants.includes(user.id)} onChange={() => toggleParticipant(user.id)}/><span>{user.name}</span><small>{user.department_name || 'Sem setor'}</small></label>)}</div></div>
          <div className="form-grid two-columns"><label className="field"><span>Duração</span><select value={duration} onChange={event => setDuration(Number(event.target.value))}><option value={30}>30 minutos</option><option value={45}>45 minutos</option><option value={60}>1 hora</option><option value={90}>1h30</option><option value={120}>2 horas</option></select></label>{mode === 'request' && <label className="field"><span>Privacidade</span><select value={privacy} onChange={event => setPrivacy(event.target.value)}><option value="normal">Normal</option><option value="private">Privado · terceiros veem apenas “Ocupado”</option></select></label>}</div>
