...
const fallbackRoles: Role[] = [
  ...
  { id: 'finance', key: 'finance', name: 'Financeiro', description: 'Operação financeira diária, sem aprovação ou execução do próprio lote.', is_system: true, is_active: true, user_count: 0, permissions: ['dashboard.view', 'properties.view', 'contracts.view', 'finance.view', 'finance.charge.create', 'finance.reconcile', 'finance.payment.prepare', 'finance.adjustment.create', 'finance.repasse.execute'] },
  { id: 'finance_approver', key: 'finance_approver', name: 'Aprovador financeiro', description: 'Aprovação de pagamentos e fechamento de competência.', is_system: true, is_active: true, user_count: 0, permissions: ['dashboard.view', 'finance.view', 'finance.payment.approve', 'finance.period.close', 'reports.view', 'audit.view'] },
  { id: 'finance_executor', key: 'finance_executor', name: 'Executor financeiro', description: 'Execução de pagamentos já aprovados.', is_system: true, is_active: true, user_count: 0, permissions: ['dashboard.view', 'finance.view', 'finance.payment.execute', 'reports.view'] },
  { id: 'finance_controller', key: 'finance_controller', name: 'Controladoria financeira', description: 'Reabertura de competência e exceções auditadas de segregação.', is_system: true, is_active: true, user_count: 0, permissions: ['dashboard.view', 'finance.view', 'finance.period.reopen', 'finance.sod.override', 'reports.view', 'reports.export', 'audit.view'] },
...
const permissionNames: Record<string, string> = {
  ...
  'finance.view': 'Visualizar financeiro', 'finance.charge.create': 'Gerar cobranças', 'finance.reconcile': 'Conciliar banco', 'finance.payment.prepare': 'Preparar pagamentos', 'finance.payment.approve': 'Aprovar pagamentos', 'finance.payment.execute': 'Executar pagamentos', 'finance.period.close': 'Fechar competência', 'finance.period.reopen': 'Reabrir competência', 'finance.adjustment.create': 'Criar ajustes financeiros', 'finance.sod.override': 'Exceção de segregação', 'finance.repasse.execute': 'Executar repasses',
  ...
  return (
    <section className="workspace settings-workspace">
      ...
      {loading ? <article className="panel settings-loading">Carregando estrutura de acesso...</article> : <>
        ...
        <article className="panel permission-panel"><div className="panel-heading"><span className="eyebrow">Governança</span><h2>Regras estruturais</h2></div><ul className="plain-list permission-rules"><li>Somente Administradores podem gerenciar usuários, permissões e alçadas.</li><li>Quem prepara um lote não o aprova; quem aprova não o executa, salvo exceção formal e auditada.</li><li>Bloquear um usuário encerra o acesso sem apagar seu histórico.</li><li>Alterações de perfil e bloqueios exigem justificativa e entram na auditoria.</li></ul></article>
      </>}
    </section>
  )
}
