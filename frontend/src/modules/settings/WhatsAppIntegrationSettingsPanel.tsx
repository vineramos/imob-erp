...
export function WhatsAppIntegrationSettingsPanel({canEdit}:{canEdit:boolean}){
  ...
  useEffect(()=>{let active=true;void apiRequest<WhatsAppConfig>('/integrations/whatsapp/config').then(data=>{if(active)setConfig(data)}).catch(cause=>{if(active)setError(cause instanceof ApiError?cause.detail:'Não foi possível carregar o WhatsApp.')}).finally(()=>{if(active)setLoading(false)});return()=>{active=false}},[])

  async function save(){
    if(!canEdit)return
    setSaving(true);setError('');setSuccess('')
    try{
      const updated=await apiRequest<WhatsAppConfig>('/integrations/whatsapp/config',{method:'PUT',body:JSON.stringify({
  ...
  async function test(){
    ...
    try{
      const result=await apiRequest<{reachable:boolean;verified_name:string|null;display_phone_number:string|null;message:string}>('/integrations/whatsapp/test',{method:'POST'})
  ...
  return <article className="panel integration-card integration-card-expanded">
    ...
    <div className="smtp-config-panel">
      <div className="form-grid smtp-config-grid">
        ...
        <label className="field"><span>WhatsApp Business Account ID</span><input disabled={!canEdit} inputMode="numeric" autoComplete="off" placeholder="WABA ID" value={config.business_account_id} onChange={e=>setConfig(current=>({...current,business_account_id:e.target.value.replace(/\D/g,'')}))}/></label>
        <label className="field"><span>Phone Number ID</span><input disabled={!canEdit} inputMode="numeric" autoComplete="off" placeholder="ID do número na Meta" value={config.phone_number_id} onChange={e=>setConfig(current=>({...current,phone_number_id:e.target.value.replace(/\D/g,'')}))}/></label>
