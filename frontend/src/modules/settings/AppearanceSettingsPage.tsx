...
export function AppearanceSettingsPage({ canEdit }: Props) {
  ...
  useEffect(() => {
    ...
    let active = true
    void apiRequest<ThemeConfig>('/settings/appearance/erp')
      .then((loadedTheme) => {
        if (active) setTheme(loadedTheme)
      })
      .catch((cause) => {
        if (active) setError(cause instanceof ApiError ? cause.detail : 'Não foi possível carregar o tema do ERP.')
      })
      .finally(() => {
  ...
  return (
    <section className="workspace settings-workspace">
      <div className="page-heading settings-heading">
        ...
        {canEdit && (
          <div className="heading-actions">
            <button className="button secondary" type="button" onClick={restoreDefault} disabled={loading || saving}>
              <RotateCcw size={16} /> Restaurar padrão
            </button>
            <button className="button primary" type="button" onClick={() => void saveTheme()} disabled={loading || saving}>
      ...
      {loading ? (
        <article className="panel settings-loading">Carregando identidade visual...</article>
      ) : (
