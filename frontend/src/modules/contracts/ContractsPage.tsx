  ...
  return <section className="workspace contracts-workspace">
    ...
    {showForm && <form className="panel portfolio-form contract-form" onSubmit={save}>
      ...
      <div className="form-grid three-columns">
        ...
        {terms.admin_fee_type === 'percent' ? <label className="field"><span>Administração (%)</span><input required type="number" min="0" max="100" step="0.01" value={terms.admin_fee_percent ?? ''} onChange={(e) => setTerms((current) => ({ ...current, admin_fee_percent: e.target.value ? Number(e.target.value) : null }))}/></label> : <label className="field"><span>Taxa fixa</span><input required type="number" min="0" step="0.01" value={terms.admin_fee_amount ?? ''} onChange={(e) => setTerms((current) => ({ ...current, admin_fee_amount: e.target.value ? Number(e.target.value) : null }))}/></label>}
        ...
        <label className="field"><span>Intermediação (%)</span><input type="number" min="0" max="500" step="0.01" value={terms.intermediation_percent} onChange={(e) => setTerms((current) => ({ ...current, intermediation_percent: Number(e.target.value) }))}/></label>
        <label className="field"><span>Parcelas intermediação</span><input type="number" min="1" max="24" value={terms.intermediation_installments} onChange={(e) => setTerms((current) => ({ ...current, intermediation_installments: Number(e.target.value) }))}/></label>
    ...
    {loading ? <article className="panel settings-loading">Carregando contratos...</article> : <div className="portfolio-card-list contract-list">{filtered.map((item) => {
      ...
      return <article className="panel contract-card" key={item.id}>
        <div className="contract-row"><div className="contract-code"><FileSignature size={18}/><span>ADMINISTRAÇÃO</span><strong>{item.code}</strong><small>v{item.current_version}</small></div><div className="contract-property"><strong>Imóvel #{item.property_code}</strong><span>{addressLine(item.property_address)}</span><small>{item.owners.map((owner) => `${owner.name} · ${Number(owner.ownership_percent).toLocaleString('pt-BR')}%`).join(' / ')}</small></div><div className="contract-commercial"><span>Plano / administração</span><strong>{planLabel[item.plan]} · {adminFee(item)}</strong><small>Intermediação {Number(item.intermediation_percent).toLocaleString('pt-BR')}% · Repasse D+{item.owner_repasse_business_days}</small></div><div className="contract-state"><i className={`status-badge ${statusClass(item.status)}`}>{statusLabel[item.status]}</i><span>{signingLabel(item)}</span></div></div>
