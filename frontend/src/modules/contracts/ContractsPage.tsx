import { CheckCircle2, ChevronDown, ChevronUp, FileSignature, History, Plus, RotateCcw, Send, ShieldCheck } from 'lucide-react'
...
const statusLabel: Record<AdministrationContractStatus, string> = {
  ...
  review: 'Em revisão',
  approved: 'Aprovado',
  pending_signature: 'Preparando assinatura',
  signed: 'Assinado',
  cancelled: 'Cancelado',
}

const planLabel: Record<string, string> = {
  essential: 'Essencial', complete: 'Completo', custom: 'Personalizado',
}

const payerLabel: Record<string, string> = {
  tenant: 'Locatário', owner: 'Proprietário', agency: 'Imobiliária',
}

const defaultTerms = (defaults?: OperationalDefaults): AdministrationContractTerms => ({
  plan: 'essential',
  admin_fee_type: 'percent',
  admin_fee_percent: defaults?.default_admin_fee_percent ?? 10,
  admin_fee_amount: null,
  intermediation_percent: 100,
  intermediation_installments: 1,
  owner_repasse_business_days: defaults?.owner_repasse_business_days ?? 2,
  condo_operational_payer: 'tenant',
  iptu_operational_payer: 'tenant',
  publication_requires_owner_approval: false,
  maintenance_limit_amount: null,
  emergency_limit_amount: null,
  start_date: null,
  end_date: null,
  notes: '',
})
...
function adminFee(item: AdministrationContract) {
  return item.admin_fee_type === 'percent'
    ? `${Number(item.admin_fee_percent ?? 0).toLocaleString('pt-BR')}%`
    : money(item.admin_fee_amount)
}
...
export function ContractsPage({ permissions }: Props) {
  ...
  function openEdit(item: AdministrationContract) {
    ...
    setTerms({
      ...
      intermediation_percent: Number(item.intermediation_percent),
      intermediation_installments: item.intermediation_installments,
      owner_repasse_business_days: item.owner_repasse_business_days,
      condo_operational_payer: item.condo_operational_payer,
      iptu_operational_payer: item.iptu_operational_payer,
      publication_requires_owner_approval: item.publication_requires_owner_approval,
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      if (editing) {
        updated = await apiRequest<AdministrationContract>(`/administration-contracts/${editing.id}`, {
          method: 'PUT',
          body: JSON.stringify({ ...terms, change_summary: changeSummary }),
        })
        ...
        setContracts((current) => [updated, ...current])
  ...
  async function workflow(item: AdministrationContract, action: AdministrationContractWorkflowAction) {
    ...
    if (action === 'cancel') {
      reason = window.prompt('Informe o motivo do cancelamento:')
    ...
    setSuccess('')
    try {
      const updated = await apiRequest<AdministrationContract>(`/administration-contracts/${item.id}/workflow`, {
        method: 'POST',
      ...
      const message: Record<AdministrationContractWorkflowAction, string> = {
        ...
        prepare_signature: `${updated.code} preparado para o provider ${updated.signing_provider}.`,
      ...
      setError(cause instanceof ApiError ? cause.detail : 'Não foi possível executar a ação no contrato.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <section className="workspace contracts-workspace">
      <div className="page-heading portfolio-heading">
        <div>
          <span className="eyebrow">Contratos · Administração</span>
          <h1>Contratos de administração</h1>
          <p>Regras próprias por imóvel, versões imutáveis e fluxo interno antes da assinatura eletrônica.</p>
        </div>
      ...
      {showForm && (
        <form className="panel portfolio-form contract-form" onSubmit={save}>
          <div className="panel-heading panel-heading-row">
            <div><span className="eyebrow">{editing ? `Nova versão · ${editing.code}` : 'Nova minuta'}</span><h2>{editing ? 'Editar termos do contrato' : 'Contrato de administração'}</h2></div>
          ...
          <div className="contract-snapshot-note"><ShieldCheck size={16}/><span>Ao salvar, proprietários e regras desta versão ficam congelados no histórico. Alterar Configurações depois não muda este contrato.</span></div>
      ...
      <div className="portfolio-toolbar panel contract-filter-bar">
          ...
          {filtered.map((item) => {
            ...
            return <article className="panel contract-card" key={item.id}>
              <div className="contract-row">
                ...
                <div className="contract-state"><i className={`status-badge ${statusClass(item.status)}`}>{statusLabel[item.status]}</i><span>{item.signing_status === 'ready_for_provider' ? `Pronto para ${item.signing_provider}` : item.status === 'signed' ? 'Documento final arquivado' : 'Fluxo interno'}</span></div>
              </div>
              ...
              {isExpanded && <div className="contract-version-history">{[...item.versions].reverse().map((version) => <div key={version.version_number}><span>v{version.version_number}</span><strong>{version.change_summary || 'Versão registrada'}</strong><small>{new Date(version.created_at).toLocaleString('pt-BR')}</small></div>)}</div>}
            </article>
          })}
      ...
      <article className="panel contract-provider-note"><FileSignature size={21}/><div><span className="eyebrow">Clicksign</span><h2>Fluxo preparado, envio externo ainda bloqueado</h2><p>“Preparar assinatura” valida a aprovação e deixa o contrato pronto para o provider. O ERP ainda não envia o envelope à Clicksign até configurarmos credenciais seguras e o webhook oficial.</p></div></article>
    </section>
  )
}
