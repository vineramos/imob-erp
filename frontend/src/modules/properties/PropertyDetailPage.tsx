  ...
  return <section className="workspace property-detail-workspace property-detail-v81" data-property-id={property.id} data-property-code={property.code}>
    ...
    {activeTab==='overview'&&<div className="property-overview-layout">
      ...
      <aside className="property-surface property-overview-summary">
        <section><div className="property-section-heading"><div><span>Responsáveis</span><h2>Gestão do ativo</h2></div></div><div className="property-people-summary property-owner-summary"><UserRound size={20}/><div><span>{property.owners.length===1?'Proprietário':'Proprietários'}</span>{property.owners.length?<div className="property-owner-list">{property.owners.map(owner=><button type="button" className="property-person-link" key={owner.person_id} onClick={()=>navigate(`/app/people/${owner.person_id}`)} title={`Abrir cadastro de ${owner.name}`}><strong>{owner.name}</strong><b>{percentLabel(owner.ownership_percent)}</b><ExternalLink size={11}/></button>)}</div>:<strong>Não vinculado</strong>}<small>{property.owners.length?`${property.owners.length} ${property.owners.length===1?'titular':'titulares'} · ${percentLabel(property.owners.reduce((total,owner)=>total+Number(owner.ownership_percent),0))} cadastrado`:'Vínculo pendente'}</small></div></div><div className="property-people-summary"><UserRound size={20}/><div><span>Corretor responsável</span><strong>Não atribuído</strong><small>Vínculo ainda não fornecido pelo cadastro</small></div></div></section>
