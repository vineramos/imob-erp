...
export function PropertyDetailPage(props:Props){
  ...
  return <section className="workspace property-detail-workspace property-detail-v81" data-property-id={property.id} data-property-code={property.code}>
    ...
    {activeTab==='finance'&&<section className="property-surface property-tab-surface"><div className="property-section-heading"><div><span>FINANCEIRO</span><h2>Valores e vínculo de locação</h2></div></div><div className="property-finance-wide"><div><span>Aluguel</span><strong>{money(property.rent_amount)}</strong></div><div><span>Condomínio</span><strong>{money(property.condo_amount)}</strong></div><div><span>IPTU</span><strong>{money(property.iptu_amount)}</strong></div><div><span>Locação vigente</span><strong>{latestLease?`${latestLease.code} · ${money(latestLease.rent_amount)}`:'Nenhuma'}</strong></div></div><PropertyAdditionalChargesPanel property={property} canEdit={props.permissions.includes('properties.edit')} onUpdated={props.onRefresh} hasSignedLease={props.leases.some(lease=>['signed','active'].includes(lease.status))}/></section>}
