  ...
  return <section className="workspace property-detail-workspace property-detail-v81" data-property-id={property.id} data-property-code={property.code}>
    ...
    {activeTab==='overview'&&<div className="property-overview-layout">
      ...
      <aside className="property-surface property-overview-summary">
        ...
        <section><div className="property-section-heading"><div><span>Financeiro</span><h2>Resumo mensal</h2></div><button type="button" onClick={()=>props.onTabChange('finance')}>Ver detalhes</button></div><div className="property-money-summary"><div className="primary"><span>Aluguel</span><strong>{money(property.rent_amount)}</strong></div><div><span>Condomínio</span><strong>{money(property.condo_amount)}</strong></div><div><span>IPTU</span><strong>{money(property.iptu_amount)}</strong></div>{(property.additional_charges||[]).filter(charge=>charge.active&&charge.include_in_invoice&&charge.payer==='tenant').map(charge=><div key={charge.key}><span>{charge.label}</span><strong>{money(Number(charge.amount||0))}</strong><small>{charge.frequency==='monthly'?'Mensal':charge.frequency==='annual'?'Anual':'Única'}</small></div>)}</div></section>
      </aside>
