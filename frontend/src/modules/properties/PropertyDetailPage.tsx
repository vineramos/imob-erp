import {
  ArrowLeft, Bath, BedDouble, Building2, CalendarPlus, Car, CheckCircle2,
  CircleAlert, CircleDollarSign, ClipboardCheck, ExternalLink, FileText,
  Globe2, History, Home, MapPin, Maximize2, Pencil, RefreshCw, Send,
  Share2, Sparkles, UserRound,
} from 'lucide-react'
...
export function PropertyDetailPage(props:Props){
  ...
  return <section className="workspace property-detail-workspace property-detail-v81" data-property-id={property.id} data-property-code={property.code}>
    ...
    <PropertyGallery propertyId={property.id} canManage={canEdit} variant="hero" heroContent={heroContent} heroActions={heroActions} onChanged={props.onRefresh}/>
    ...
    {activeTab==='overview'&&<div className="property-overview-layout">
      <main className="property-overview-main">
        <section className="property-surface property-description"><div className="property-section-heading"><div><span>O ATIVO</span><h2>Visão geral</h2></div>{canEdit&&<button type="button" onClick={()=>props.setCommercialEditing(true)}><Pencil size={14}/> Editar apresentação</button>}</div><p>{commercialProfile?.public_description||'A descrição comercial deste imóvel ainda não foi preenchida.'}</p><div className="property-overview-facts"><span><Building2 size={17}/>{propertyTypes[property.property_type]??property.property_type}</span><span><Maximize2 size={17}/>{property.area_m2??'—'} m²</span><span><BedDouble size={17}/>{property.bedrooms} dormitórios</span><span><Bath size={17}/>{property.bathrooms} banheiros</span><span><Car size={17}/>{property.parking_spaces} vagas</span><span><Sparkles size={17}/>{property.furnished?'Mobiliado':'Não mobiliado'}</span></div></section>
        ...
        <section className="property-surface"><div className="property-section-heading"><div><span>OPERAÇÃO</span><h2>Andamento comercial</h2></div></div><div className="property-commercial-flow"><div className="done"><CheckCircle2 size={18}/><span><strong>Cadastro</strong><small>Ativo criado em {dateLabel(property.created_at)}</small></span></div><div className={ownerNames?'done':''}><CheckCircle2 size={18}/><span><strong>Titularidade</strong><small>{ownerNames||'Proprietário pendente'}</small></span></div><div className={readiness?.ready?'done':'attention'}><CircleAlert size={18}/><span><strong>Preparação comercial</strong><small>{readiness?.ready?'Checklist concluído':`${requiredPending} pendência(s)`}</small></span></div><div className={property.publication_enabled?'done':''}><Globe2 size={18}/><span><strong>Publicação</strong><small>{property.publication_enabled?'Anúncio ativo':'Ainda não publicado'}</small></span></div></div></section>
      </main>
      <aside className="property-overview-aside">
        <section className="property-surface"><div className="property-section-heading"><div><span>RESPONSÁVEIS</span><h2>Pessoas vinculadas</h2></div></div><div className="property-people-summary"><UserRound size={20}/><div><span>Proprietário</span><strong>{ownerNames||'Não vinculado'}</strong><small>{property.owners.length?`${property.owners.length} titular(es)`:'Vínculo pendente'}</small></div></div><div className="property-people-summary"><UserRound size={20}/><div><span>Corretor responsável</span><strong>Não atribuído</strong><small>O cadastro atual ainda não fornece este vínculo</small></div></div></section>
        <section className="property-surface"><div className="property-section-heading"><div><span>RESUMO</span><h2>Financeiro</h2></div><button type="button" onClick={()=>props.onTabChange('finance')}>Ver detalhes</button></div><div className="property-money-summary"><div><span>Aluguel</span><strong>{money(property.rent_amount)}</strong></div><div><span>Condomínio</span><strong>{money(property.condo_amount)}</strong></div><div><span>IPTU</span><strong>{money(property.iptu_amount)}</strong></div></div></section>
        <section className="property-surface"><div className="property-section-heading"><div><span>ATIVIDADE</span><h2>Movimentações recentes</h2></div></div><ActivityTimeline items={[{id:'updated',title:'Cadastro atualizado',timestamp:dateLabel(property.updated_at),icon:RefreshCw},{id:'created',title:'Imóvel cadastrado',timestamp:dateLabel(property.created_at),icon:History},...(property.published_at?[{id:'published',title:'Anúncio publicado',timestamp:dateLabel(property.published_at),icon:Globe2}]:[])]}/></section>
        <section className="property-surface property-publication-summary"><div className="property-section-heading"><div><span>PUBLICAÇÃO</span><h2>Presença digital</h2></div></div><div><Globe2 size={20}/><span><strong>{property.publication_enabled?'Publicado no site':'Anúncio inativo'}</strong><small>{readiness?.ready?'Imóvel pronto para publicação':`${requiredPending} item(ns) pendente(s)`}</small></span></div>{canPublish&&<button className="button primary" type="button" disabled={props.saving||(!property.publication_enabled&&Boolean(readiness&&!readiness.ready))} onClick={()=>props.onTogglePublication(!property.publication_enabled)}>{property.publication_enabled?<><Globe2 size={14}/> Retirar do site</>:<><Send size={14}/> Publicar imóvel</>}</button>}</section>
      </aside>
    </div>}
