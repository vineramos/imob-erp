...
export function PropertyDetailPage(props:Props){
  ...
  return <section className="workspace property-detail-workspace property-detail-v81" data-property-id={property.id} data-property-code={property.code}>
    ...
    {activeTab==='overview'&&<div className="property-overview-layout">
      ...
      <div className="property-overview-middle">
        <section className="property-surface property-main-information"><div className="property-section-heading"><div><span>Informações principais</span><h2>Características do imóvel</h2></div></div><div className="property-overview-facts"><span><Building2 size={17}/><small>Tipo</small><strong>{propertyTypes[property.property_type]??property.property_type}</strong></span><span><Maximize2 size={17}/><small>Área</small><strong>{property.area_m2??'—'} m²</strong></span><span><BedDouble size={17}/><small>Dormitórios</small><strong>{property.bedrooms}</strong></span><span><Bath size={17}/><small>Banheiros</small><strong>{property.bathrooms}</strong></span><span><Car size={17}/><small>Vagas</small><strong>{property.parking_spaces}</strong></span><span><Sparkles size={17}/><small>Mobília</small><strong>{property.furnished?'Mobiliado':'Não mobiliado'}</strong></span></div></section>
        <section className="property-surface property-progress-surface"><div className="property-section-heading"><div><span>Comercial</span><h2>Andamento do imóvel</h2></div></div><div className="property-commercial-flow"><div className="done"><CheckCircle2 size={18}/><span><strong>Cadastro</strong><small>Ativo criado em {dateLabel(property.created_at)}</small></span></div><div className={ownerNames?'done':''}><CheckCircle2 size={18}/><span><strong>Titularidade</strong><small>{ownerNames||'Proprietário pendente'}</small></span></div><div className={readiness?.ready?'done':'attention'}><CircleAlert size={18}/><span><strong>Preparação</strong><small>{readiness?.ready?'Checklist concluído':`${requiredPending} pendência(s)`}</small></span></div><div className={property.publication_enabled?'done':'current'}><Globe2 size={18}/><span><strong>Publicação</strong><small>{property.publication_enabled?'Anúncio ativo':'Ainda não publicado'}</small></span></div></div></section>
      </div>
      <aside className="property-overview-aside">
        <section className="property-surface property-rail-section"><div className="property-section-heading"><div><span>Responsáveis</span><h2>Pessoas vinculadas</h2></div></div><div className="property-people-summary"><UserRound size={20}/><div><span>Proprietário</span><strong>{ownerNames||'Não vinculado'}</strong><small>{property.owners.length?`${property.owners.length} titular(es)`:'Vínculo pendente'}</small></div></div><div className="property-people-summary"><UserRound size={20}/><div><span>Corretor responsável</span><strong>Não atribuído</strong><small>O cadastro atual ainda não fornece este vínculo</small></div></div></section>
        <section className="property-surface property-rail-section"><div className="property-section-heading"><div><span>Financeiro</span><h2>Resumo mensal</h2></div><button type="button" onClick={()=>props.onTabChange('finance')}>Ver detalhes</button></div><div className="property-money-summary"><div className="primary"><span>Aluguel</span><strong>{money(property.rent_amount)}</strong></div><div><span>Condomínio</span><strong>{money(property.condo_amount)}</strong></div><div><span>IPTU</span><strong>{money(property.iptu_amount)}</strong></div></div></section>
        <section className="property-surface property-rail-section"><div className="property-section-heading"><div><span>Atividades</span><h2>Movimentações recentes</h2></div></div><ActivityTimeline items={[{id:'updated',title:'Cadastro atualizado',timestamp:dateLabel(property.updated_at),icon:RefreshCw},{id:'created',title:'Imóvel cadastrado',timestamp:dateLabel(property.created_at),icon:History},...(property.published_at?[{id:'published',title:'Anúncio publicado',timestamp:dateLabel(property.published_at),icon:Globe2}]:[])]}/></section>
        <section className="property-surface property-publication-summary property-rail-section"><div className="property-section-heading"><div><span>Publicação</span><h2>Presença digital</h2></div></div><div><Globe2 size={20}/><span><strong>{property.publication_enabled?'Publicado no site':'Anúncio inativo'}</strong><small>{readiness?.ready?'Imóvel pronto para publicação':`${requiredPending} item(ns) pendente(s)`}</small></span></div>{canPublish&&<button className="button primary" type="button" disabled={props.saving||(!property.publication_enabled&&Boolean(readiness&&!readiness.ready))} onClick={()=>props.onTogglePublication(!property.publication_enabled)}>{property.publication_enabled?<><Globe2 size={14}/> Retirar do site</>:<><Send size={14}/> Publicar imóvel</>}</button>}</section>
      </aside>
    </div>}
