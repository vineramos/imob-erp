...
type EditableCharge=Omit<Charge,'amount'|'agency_retention_value'>&{amount:number|string;agency_retention_value:number|string}
// Preserve intermediate input (including a trailing comma) until the user saves.
...
export function PropertyAdditionalChargesPanel({property,canEdit,onUpdated,hasSignedLease}:Props){
 const [rows,setRows]=useState<EditableCharge[]>(property.additional_charges||[])
 ...
 const patch=(key:string,data:Partial<EditableCharge>)=>{setInvalidKeys(old=>old.filter(item=>item!==key));setRows(old=>old.map(item=>item.key===key?{...item,...data}:item))}
 async function save(){
   setError('');setSaved('')
   const invalid=rows.filter(row=>!row.label.trim()||parseDecimal(row.amount)===null|| (row.agency_retention_type!=='none'&&parseDecimal(row.agency_retention_value)===null))
   if(invalid.length){setInvalidKeys(invalid.map(row=>row.key));setError('Confira os campos destacados. Use, por exemplo, 100,00.');return}
   setInvalidKeys([])
   setBusy(true)
   try{
     const updated=await apiRequest<Property>(`/properties/${property.id}/additional-charges`,{method:'PUT',body:JSON.stringify({charges:rows.map(row=>({...row,amount:parseDecimal(row.amount),agency_retention_value:row.agency_retention_type==='none'?0:parseDecimal(row.agency_retention_value),label:row.label.trim(),beneficiary_name:row.beneficiary_name?.trim()||null}))})})
 ...
 {canEdit&&!editing&&<button type="button" className="button secondary" onClick={()=>{setEditing(true);setSaved('');setInvalidKeys([])}}>Editar encargos</button>}</div>
 ...
 {editing&&<div className="property-charge-actions">
  ...
  <button type="button" className="button secondary" disabled={busy} onClick={()=>{setEditing(false);setRows(property.additional_charges||[]);setError('');setInvalidKeys([])}}>Cancelar</button>
