...
type ClientLease = {
  ...
  tenants:Array<{person_id:string;name:string}>; owners:Array<{person_id:string;name:string;ownership_percent:string|number}>;
...
export function PeopleWorkspacePage({ permissions }: { permissions: string[] }) {
  ...
  const propertyStatus=(status:string)=>({draft:'Rascunho',available:'Disponível',reserved:'Reservado',leased:'Locado',inactive:'Inativo'}[status]||status)
  const ownershipForSelected=(property:Property)=>Number(property.owners.find(owner=>owner.person_id===selected?.id)?.ownership_percent||0)
  function openEntity(route:string){window.history.pushState({},'',route);window.dispatchEvent(new PopStateEvent('popstate'))}
      ...
      <main className="panel client-profile">
        {!selected?<div className="client-profile-empty"><Users size={28}/><strong>Selecione um cliente</strong><span>A ficha completa aparecerá aqui.</span></div>:<>
          ...
          <div className="client-profile-body">
            ...
            {clientTab==='properties'&&<div className="client-tab-section client-properties-tab"><div className="client-tab-heading"><div><span className="eyebrow">Patrimônio e vínculos</span><h3>Imóveis</h3></div><strong>{selectedProperties.length}</strong></div>{selectedProperties.length?<div className="client-property-list">{selectedProperties.map(property=><article key={property.id} className="client-property-card"><button type="button" className="client-property-main" onClick={()=>openEntity(`/app/properties/${property.id}`)}><div><span className="eyebrow">{property.code}</span><strong>{property.public_title||propertyAddress(property)}</strong><small>{propertyAddress(property)}</small></div><div className="client-property-summary"><strong>{money(property.rent_amount)}</strong><i className={'status-badge '+(property.status==='available'?'success':property.status==='inactive'?'neutral':'warning')}>{propertyStatus(property.status)}</i></div></button><div className="client-owner-share"><div className="client-owner-share-head"><span>Sua participação</span><strong>{ownershipForSelected(property).toLocaleString('pt-BR',{maximumFractionDigits:2})}%</strong></div><div className="client-coowners">{property.owners.map(owner=><button type="button" key={owner.person_id} className={owner.person_id===selected.id?'current':''} onClick={()=>openEntity(`/app/people/${owner.person_id}`)}><span>{owner.name}</span><strong>{Number(owner.ownership_percent).toLocaleString('pt-BR',{maximumFractionDigits:2})}%</strong></button>)}</div></div></article>)}</div>:<div className="client-tab-empty"><Home size={22}/>Nenhum imóvel vinculado a este cliente.</div>}</div>}

            {clientTab==='contracts'&&<div className="client-tab-section client-contracts-tab"><div className="client-tab-heading"><div><span className="eyebrow">Relação contratual</span><h3>Contratos</h3></div><strong>{selectedLeases.length}</strong></div>{selectedLeases.length?<div className="client-contract-list">{selectedLeases.map(lease=>{const isTenant=lease.tenants.some(tenant=>tenant.person_id===selected.id);return <article key={lease.id} className="client-contract-card"><button type="button" className="client-contract-main" onClick={()=>openEntity(`/app/contracts/lease/${lease.id}`)}><div><span>{lease.code} · imóvel #{lease.property_code}</span><strong>{[lease.property_address?.street,lease.property_address?.number].filter(Boolean).join(', ')||'Imóvel vinculado'}</strong><small>{isTenant?'Locatário':'Proprietário'} · {lease.start_date?new Date(lease.start_date+'T12:00:00').toLocaleDateString('pt-BR'):'—'} até {lease.end_date?new Date(lease.end_date+'T12:00:00').toLocaleDateString('pt-BR'):'—'}</small></div><div><strong>{money(lease.rent_amount)}</strong><i className={'status-badge '+(['signed','approved'].includes(lease.status)?'success':['cancelled'].includes(lease.status)?'danger':'neutral')}>{leaseStatus(lease.status)}</i></div></button><div className="client-contract-parties"><span>{isTenant?'Proprietários':'Locatários'}</span><div>{(isTenant?lease.owners:lease.tenants).map(party=><button type="button" key={party.person_id} onClick={()=>openEntity(`/app/people/${party.person_id}`)}><strong>{party.name}</strong>{'ownership_percent' in party&&<small>{Number(party.ownership_percent).toLocaleString('pt-BR',{maximumFractionDigits:2})}%</small>}</button>)}</div></div></article>})}</div>:<div className="client-tab-empty"><FileText size={22}/>Nenhum contrato vinculado a este cliente.</div>}</div>}
