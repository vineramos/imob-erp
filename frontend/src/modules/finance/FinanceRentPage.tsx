...
type Charge = {
  id:string; code:string; lease_contract_id:string; lease_code:string; property_id:string; property_code:string; property_address:Record<string,string>;
  tenants:Array<{name?:string}>; owners:Array<{name?:string}>; competence:string; due_date:string; status:string; days_overdue:number; critical_overdue:boolean;
  rent_amount:number; gross_amount:number; updated_amount:number; amount_as_of:string; late_fee_percent:number; late_interest_percent_monthly:number;
  late_interest_type:'simple'|'compound'; late_interest_compounding:'daily'|'monthly'; late_fee_amount:number; late_interest_amount:number;
...
function interestLabel(item:Charge){return item.late_interest_type==='compound'?`composto · cap. ${item.late_interest_compounding==='daily'?'diária':'mensal'}`:'simples'}
...
export function FinancePage({permissions}:Props){
  ...
  async function receive(event:FormEvent){
    ...
    setSaving(true);setError('');setSuccess('')
    try{
      await apiRequest(`/finance/charges/${paymentCharge.id}/payment`,{method:'POST',body:JSON.stringify({paid_amount:paymentCharge.updated_amount,paid_at:new Date(paymentAt).toISOString(),payment_method:paymentMethod,payment_reference:paymentReference||null})})
      const code=paymentCharge.code;setPaymentCharge(null);setPaymentReference('');await load();setSuccess(`${code} recebida integralmente. Multa/juros contratuais, taxas e repasses foram liquidados pela mesma memória de cálculo.`)
  ...
  return <section className="workspace finance-workspace">
    ...
    {dashboard&&<div className="finance-metrics">
      ...
      <article className="panel finance-metric"><span>Em aberto</span><strong>{money(dashboard.open_amount)}</strong><small>{dashboard.charges_open} cobrança(s) · valor atualizado hoje</small></article>
      <article className={`panel finance-metric ${dashboard.charges_critical?'critical':''}`}><span>Inadimplência</span><strong>{money(dashboard.overdue_amount)}</strong><small>{dashboard.charges_critical?`${dashboard.charges_critical} caso(s) crítico(s)`:`${dashboard.charges_overdue} em atraso`} · com mora do dia</small></article>
    ...
    {loading?<article className="panel settings-loading">Carregando financeiro...</article>:tab==='charges'?<>
      ...
      <div className="finance-charge-list">{filtered.map(item=><article className={`panel finance-charge ${item.critical_overdue?'critical':''}`} key={item.id}>
        <div className="finance-charge-top"><div className="finance-charge-code"><CircleDollarSign size={18}/><div><span>{item.code}</span><strong>{item.lease_code} · Imóvel #{item.property_code}</strong><small>{addressLine(item.property_address)}</small></div></div><div className="finance-charge-tenant"><span>Locatário(s)</span><strong>{item.tenants.map(t=>t.name).filter(Boolean).join(' / ')||'—'}</strong><small>Competência {item.competence.slice(0,7).split('-').reverse().join('/')}</small></div><div className="finance-charge-due"><span>Vencimento</span><strong>{dateLabel(item.due_date)}</strong>{item.days_overdue>0&&<small>{item.days_overdue} dia(s) em atraso</small>}</div><div className="finance-charge-total"><span>{item.days_overdue>0?'Total atualizado':'Total'}</span><strong>{money(item.status==='paid'?(item.paid_amount??item.updated_amount):item.updated_amount)}</strong>{item.days_overdue>0&&<small>Nominal {money(item.gross_amount)}</small>}<i className={`status-badge ${item.status==='paid'?'success':item.status==='overdue'?'danger':item.status==='cancelled'?'neutral':'warning'}`}>{statusLabels[item.status]||item.status}</i></div></div>
        <div className="finance-charge-bottom"><div className="finance-components">{item.charge_items.map(component=><span key={component.key}>{component.label} <b>{money(component.amount)}</b>{Number(component.agency_retention_amount||0)>0&&<small> · retém {money(component.agency_retention_amount)} · repassa {money(component.third_party_net_amount)}</small>}</span>)}{Number(item.late_fee_amount)>0&&<span>Multa {Number(item.late_fee_percent).toLocaleString('pt-BR')}% <b>{money(item.late_fee_amount)}</b></span>}{Number(item.late_interest_amount)>0&&<span>Juros {Number(item.late_interest_percent_monthly).toLocaleString('pt-BR')}% a.m. <b>{money(item.late_interest_amount)}</b><small> · {interestLabel(item)} · cálculo até {dateLabel(item.amount_as_of)}</small></span>}</div>{item.settlement&&<div className="finance-settlement-mini"><span>Taxa adm. <b>{money(item.settlement.admin_fee_calculated)}</b></span><span>Intermediação <b>{money(item.settlement.intermediation_fee_calculated)}</b></span>{Number(item.settlement.agency_retention_amount||0)>0&&<span>Retenção seguros <b>{money(item.settlement.agency_retention_amount)}</b></span>}<span>Terceiros <b>{money(item.settlement.third_party_amount)}</b></span><span>Direito proprietário <b>{money(item.settlement.owner_entitlement_amount)}</b></span></div>}<div className="finance-actions">{canGenerate&&['generated','overdue'].includes(item.status)&&<button className="button secondary compact" type="button" disabled={saving} onClick={()=>void markSent(item)}><Send size={13}/> Marcar enviada</button>}{canReconcile&&['generated','sent','overdue'].includes(item.status)&&<button className="button primary compact" type="button" disabled={saving} onClick={()=>{setPaymentCharge(item);setPaymentAt(localDateTime());setPaymentMethod('pix');setPaymentReference('')}}><CheckCircle2 size={13}/> Receber</button>}</div></div>
