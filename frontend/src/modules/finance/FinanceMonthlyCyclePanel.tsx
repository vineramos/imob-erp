...
type ClosureEvent={id:string;action:'closed'|'reopened';reason:string|null;actor_user_id:string|null;actor_name:string|null;created_at:string}
  ...
  return <section className="workspace finance-cycle-workspace">
    ...
    {loading&&!cycle?<article className="panel settings-loading">Carregando ciclo financeiro...</article>:cycle&&<>
      ...
      {readiness&&<article className={`panel finance-closing-readiness ${readiness.can_close?'ready':'blocked'}`}>
        ...
        <div className="finance-closing-actions">{!readiness.can_close&&<><button className="button secondary compact" type="button" onClick={()=>navigate('banking')}>Abrir conciliação</button><button className="button secondary compact" type="button" onClick={()=>navigate('treasury')}>Abrir tesouraria</button><button className="button secondary compact" type="button" onClick={()=>navigate('bank-control')}>Controle bancário</button></>}{permissions.includes('finance.period.close')&&closure?.status==='open'&&readiness.can_close&&<button className="button primary compact" type="button" disabled={saving} onClick={()=>void closeCompetence()}>Fechar competência</button>}{permissions.includes('finance.period.reopen')&&closure?.status==='closed'&&<button className="button secondary compact" type="button" disabled={saving} onClick={()=>void reopenCompetence()}>Reabrir competência</button>}</div>
        ...
        {closure&&closure.events.length>0&&<details className="finance-closure-history"><summary>Histórico de fechamento ({closure.events.length})</summary>{closure.events.map(item=><div key={item.id}><strong>{item.action==='closed'?'Fechada':'Reaberta'}</strong><span>{item.actor_name||'Usuário'} · {new Date(item.created_at).toLocaleString('pt-BR')}</span>{item.reason&&<small>{item.reason}</small>}</div>)}</details>}
      </article>}
