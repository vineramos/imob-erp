import { AlertTriangle, Banknote, CheckCircle2, ChevronLeft, ChevronRight, FileClock, ReceiptText, RefreshCw, Send, ShieldAlert } from 'lucide-react'
...
export function FinanceBillingPanel({permissions}:{permissions:string[]}){
  ...
  async function confirmReceipt(item:BillingItem){
    const amount=window.prompt('Valor efetivamente recebido (R$):',Number(item.amount||0).toFixed(2).replace('.',','))
    if(amount===null)return
    const normalized=Number(amount.replace(/\./g,'').replace(',','.'))
    if(!Number.isFinite(normalized)||normalized<=0){setError('Informe um valor de recebimento válido.');return}
    const reference=window.prompt('Referência do recebimento (opcional):')||null
    if(!batch)return
    setSaving(true);setError('');setSuccess('')
    try{await apiRequest(`/finance/advanced/billing/batches/${batch.id}/items/${item.id}/confirm-receipt`,{method:'POST',body:JSON.stringify({paid_amount:normalized,payment_method:'manual',payment_reference:reference,notes:'Recebimento confirmado manualmente.'})});setSuccess(`${item.charge_code} recebido. Settlement e repasse foram recalculados.`);await load()}catch(e){setError(e instanceof ApiError?e.detail:'Não foi possível confirmar o recebimento.')}finally{setSaving(false)}
  }
