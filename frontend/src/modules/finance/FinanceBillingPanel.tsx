import { AlertTriangle, Banknote, CheckCircle2, FileClock, RefreshCw, Send, ShieldAlert } from 'lucide-react'
...
export function FinanceBillingPanel({permissions}:{permissions:string[]}){
  ...
  return <section className="workspace finance-advanced-workspace">
    <div className="page-heading finance-heading"><div><span className="eyebrow">Financeiro · Cobranças</span><h1>Cobrança mensal e inadimplência</h1><p>Geração em lote, boleto/Pix, envio, confirmação e tratamento de atraso no mesmo ciclo.</p></div><div className="heading-actions"><input className="finance-compact-input" type="month" value={month} onChange={e=>setMonth(e.target.value)}/><button className="button secondary" onClick={()=>void load()} disabled={loading}><RefreshCw size={14}/> Atualizar</button>{canCharge&&<button className="button primary" onClick={()=>void run()} disabled={saving}><Banknote size={14}/> Gerar mês</button>}</div></div>
