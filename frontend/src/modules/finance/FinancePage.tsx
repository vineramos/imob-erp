...
type Settlement = { id:string; administration_contract_id:string|null; admin_fee_calculated:number; intermediation_fee_calculated:number; agency_fee_withheld:number; agency_reimbursement_amount:number; owner_entitlement_amount:number; third_party_amount:number; calculated_at:string; repasses:Repasse[] }
...
export function FinancePage({permissions}:Props){
  ...
  async function receive(event:FormEvent){
    ...
    try{
      ...
      const code=paymentCharge.code;setPaymentCharge(null);setPaymentReference('');await load();setSuccess(`${code} recebida integralmente. Taxas e repasses foram calculados automaticamente.`)
  ...
  return <section className="workspace finance-workspace">
    ...
    {loading?<article className="panel settings-loading">Carregando financeiro...</article>:tab==='charges'?<>
      ...
      <div className="finance-charge-list">{filtered.map(item=><article className={`panel finance-charge ${item.critical_overdue?'critical':''}`} key={item.id}>
        ...
        <div className="finance-charge-bottom"><div className="finance-components">{item.charge_items.map(component=><span key={component.key}>{component.label} <b>{money(component.amount)}</b></span>)}</div>{item.settlement&&<div className="finance-settlement-mini"><span>Taxa adm. <b>{money(item.settlement.admin_fee_calculated)}</b></span><span>Intermediação <b>{money(item.settlement.intermediation_fee_calculated)}</b></span><span>Direito proprietário <b>{money(item.settlement.owner_entitlement_amount)}</b></span></div>}<div className="finance-actions">{canGenerate&&['generated','overdue'].includes(item.status)&&<button className="button secondary compact" type="button" disabled={saving} onClick={()=>void markSent(item)}><Send size={13}/> Marcar enviada</button>}{canReconcile&&['generated','sent','overdue'].includes(item.status)&&<button className="button primary compact" type="button" disabled={saving} onClick={()=>{setPaymentCharge(item);setPaymentAt(localDateTime());setPaymentMethod('pix');setPaymentReference('')}}><CheckCircle2 size={13}/> Receber</button>}</div></div>
