import { BarChart3, ExternalLink, Landmark, LayoutDashboard, Percent, ReceiptText, Send, ShieldCheck, Tags, TrendingUp, WalletCards } from 'lucide-react'
...
type Area = 'overview'|'billing'|'treasury'|'banking'|'inter'|'reports'|'commissions'|'classifications'|'portals'|'rent'|'maintenance'
...
export function FinancePage({permissions}:{permissions:string[]}){
  ...
  {area==='overview'?<FinanceCorePanel permissions={permissions} onNavigateSource={source=>setArea(source==='maintenance'?'maintenance':'rent')}/>:area==='billing'?<FinanceBillingPanel permissions={permissions}/>:area==='treasury'?<FinanceTreasuryPanel permissions={permissions}/>:area==='banking'?<FinanceBankingPanel permissions={permissions}/>:area==='inter'?<FinanceInterPanel permissions={permissions}/>:area==='reports'?<FinanceReportsPanel/>:area==='commissions'?<FinanceCommissionsPanel permissions={permissions}/>:area==='classifications'?<FinanceClassificationsPanel permissions={permissions}/>:area==='portals'?<FinancePortalsPanel permissions={permissions}/>:area==='rent'?<FinanceRentPage permissions={permissions}/>:<MaintenanceFinancePanel permissions={permissions}/>}</>
}
