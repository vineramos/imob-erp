...
export function FinanceCommissionsPanel({permissions}:{permissions:string[]}){
 ...
 <div className="commission-view-tabs" role="group" aria-label="Fluxo de comissões">
  <button type="button" className={view==='entries'?'active':''} onClick={()=>{setView('entries');setSelectedId(null);setQuery('')}}>Comissões geradas <span>{entries.length}</span></button>
  <button type="button" className={view==='batches'?'active':''} onClick={()=>{setView('batches');setSelectedId(null);setQuery('')}}>Lotes e notas fiscais <span>{currentBatches.length}</span></button>
  <button type="button" className={view==='rules'?'active':''} onClick={()=>{setView('rules');setSelectedId(null);setQuery('')}}>Regras <span>{rules.length}</span></button>
 </div>
 <div className="commission-master-detail">
  <aside className="panel commission-directory">
   <label className="commission-search"><Search size={15}/><input aria-label="Pesquisar comissões" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar beneficiário, código ou origem..."/></label>
   ...
   <div className="commission-directory-list">
    {view==='entries'&&visibleEntries.map(e=><button key={e.id} type="button" aria-pressed={activeEntry?.id===e.id} className={'commission-directory-item '+(activeEntry?.id===e.id?'active':'')} onClick={()=>setSelectedId(e.id)}>
      <span className="commission-item-row"><strong>{e.beneficiary_name}</strong><i className={'status-badge '+(e.status==='paid'?'success':e.status==='cancelled'?'neutral':'warning')}>{e.status}</i></span>
  ...
  <section className="panel commission-detail" aria-label="Ficha da comissão">
   {loading?<div className="commission-empty">Carregando...</div>:view==='entries'&&activeEntry?<>
    <header className="commission-detail-header"><div><span className="eyebrow">COMISSÃO · {activeEntry.code}</span><h2>{activeEntry.beneficiary_name}</h2><p>Origem: {activeEntry.source_code}</p></div><i className={'status-badge '+(activeEntry.status==='paid'?'success':'warning')}>{activeEntry.status}</i></header>
    <div className="commission-detail-strip"><div><span>Comissão</span><strong>{money(activeEntry.amount)}</strong></div><div><span>Base de cálculo</span><strong>{money(activeEntry.basis_amount)}</strong></div><div><span>Vencimento</span><strong>{activeEntry.due_date.split('-').reverse().join('/')}</strong></div></div>
    <div className="commission-detail-body"><article className="commission-detail-card"><h3>Origem e liquidação</h3><div className="commission-detail-facts"><div><span>Beneficiário</span><strong>{activeEntry.beneficiary_name}</strong></div><div><span>Origem</span><strong>{activeEntry.source_code}</strong></div><div><span>Situação</span><strong>{activeEntry.status}</strong></div><div><span>Baixa financeira</span><strong>{activeEntry.paid_at?new Date(activeEntry.paid_at).toLocaleString('pt-BR'):'Pendente'}</strong></div><div><span>Título financeiro</span><strong>{activeEntry.financial_title_id?'Vinculado':'Não vinculado'}</strong></div></div></article>
