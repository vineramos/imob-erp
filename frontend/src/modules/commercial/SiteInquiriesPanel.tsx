...
type SiteInquiry = {
  ...
  property_broker_person_id: string | null
  property_broker_name: string | null
...
type CommercialFunnel = {
  ...
  property_broker: { id:string;name:string } | null
...
export function SiteInquiriesPanel({ permissions }: Props) {
  ...
  async function loadFunnel(inquiryId: string, resetMessage = true) {
    ...
    try {
      const [next,messages]=await Promise.all([
        apiRequest<CommercialFunnel>(`/crm/site-inquiries/${inquiryId}/funnel`),
        apiRequest<WhatsAppConversationMessage[]>(`/crm/site-inquiries/${inquiryId}/whatsapp/messages`).catch(()=>[]),
      ])
      setFunnel(next);setWhatsappMessages(messages)
      setItems((current) => current.map((row) => row.id === inquiryId ? { ...row, ...next.inquiry } : row)); setWorkflowStage(next.inquiry.status==='converted'?'proposal':next.inquiry.status as FunnelStageStatus);setWorkflowResponsible(next.inquiry.responsible_user_id||'');setWorkflowAction(next.inquiry.next_action_title||'');setWorkflowAt(toLocalInput(next.inquiry.next_action_at));setWorkflowNotes(next.inquiry.next_action_notes||''); if (resetMessage) setProposalRent(next.suggested_rent_amount == null ? null : Number(next.suggested_rent_amount))
    }
    catch (cause) { setModalError(cause instanceof ApiError ? cause.detail : 'Não foi possível abrir o atendimento comercial.') }
    finally { setModalLoading(false) }
  }
  async function openFunnel(inquiryId: string) { setSelectedId(inquiryId); setFunnel(null); setWhatsappMessages([]); setWhatsappReply(''); setVisitOpen(false); setProposalOpen(false); setProposalRent(null); setCompositionProposal(null); setComposition(null); await loadFunnel(inquiryId) }
  ...
  async function sendWhatsAppReply(event: FormEvent) {
    ...
    setWhatsappSending(true);setModalError('');setModalSuccess('')
    try{
