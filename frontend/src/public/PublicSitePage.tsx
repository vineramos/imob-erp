...
function HeroRentRangeControl({ minRent, maxRent, onMinRent, onMaxRent }: RentRangeProps) {
  return <div className="public-hero-rent-filter">
    <span className="public-hero-rent-title">Valor do aluguel</span>
    <div className="public-hero-rent-fields">
      <div className="public-hero-rent-field">
        <span>De R$</span>
        <input aria-label="Valor mínimo" inputMode="numeric" type="number" min={0} step={100} value={minRent || ''} placeholder="0" onChange={(event) => {
          const value = Math.max(0, Number(event.target.value || 0))
          onMinRent(value)
          if (maxRent > 0 && value > maxRent) onMaxRent(0)
        }}/>
      </div>
      <span className="public-hero-rent-divider" aria-hidden="true"/>
      <div className="public-hero-rent-field">
        <span>Até R$</span>
        <input aria-label="Valor máximo" inputMode="numeric" type="number" min={minRent} step={100} value={maxRent || ''} placeholder="Sem limite" onChange={(event) => onMaxRent(Math.max(0, Number(event.target.value || 0)))}/>
      </div>
    </div>
  </div>
}
...
function SearchResultCard({ organizationId, item }: { organizationId: string; item: PublicProperty }) {
  ...
  return <a className="public-search-result-card" href={`/site/${organizationId}/imoveis/${item.slug}`}>
    ...
    <div className="public-search-result-copy">
      ...
      <div className="public-search-result-price"><small>Aluguel mensal</small><strong>{money(searchedValue)}</strong><div className="public-search-result-costs"><span>Aluguel <b>{money(item.rent_amount)}</b></span><span>Condomínio <b>{money(item.condo_amount)}</b></span><span>IPTU <b>{money(item.iptu_amount)}</b></span>{(item.additional_charges||[]).slice(0,3).map((charge,index)=><span key={charge.label+'-'+index}>{charge.label} <b>{money(charge.amount)}</b></span>)}</div></div>
